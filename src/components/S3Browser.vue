  ...
  <div class="s3-browser">
    ...
    <div v-if="isSearchActive" class="search-results">
      ...
      <div class="video-grid">
        <!-- 目录项 -->
        <div v-for="result in searchResults.filter((r) => r.IsDirectory)" :key="result.Key" class="folder-card"
        ...
        <!-- 视频和文件项 -->
        <div v-for="result in searchResults.filter((r) => !r.IsDirectory)" :key="result.Key || result.id"
          class="video-card" @click="handleSearchResultClick(result)">
          <div class="thumbnail-container">
            <img v-if="result.thumbnail_url || result.thumbnailUrl" :src="result.thumbnail_url || result.thumbnailUrl"
              class="thumbnail-image" :alt="result.video_title || result.name" loading="lazy" />
            ...
            <div class="video-overlay">
              ...
              <div class="video-meta-overlay">
                ...
                <span class="views" v-if="result.video_views || result.views">
                  ...
                  {{ (result.video_views || result.views) }}
                </span>
              </div>
            </div>
          </div>
